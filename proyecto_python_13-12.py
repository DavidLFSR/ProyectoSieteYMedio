...
def puede_pedir(lista_de_cartas,puntuacion_cartas):
    ...
    if cantidad_si_muerte>cantidad_no_muerte:
        return False
    else:
        return True
...
def usuarios():
    ...
    if bots=='NO':
        ...
        jugadores=[]
        while True:
            numj=int(input(('Cuantos jugadores humanos(max{}): '.format((max_j-1)))))
            if auto_mode:
                min_j_h=0
            else:
                min_j_h=1
            ...
            if jugador[i][3]==0:
                ...
                if i[:3]=='bot':
                    ...
                    while True:
                        ...
                        if jugador[i][1]=='plantado':
                            break
                elif puede_pedir(cartas,jugador[i][4]):
                    apostado=False
                    min=jugador[i][6]//5
                    ...
                    while True and not apostado:
                        ...
                        if not isinstance(jugador[i][5],int):
                            ...
                            break
                    while True and jugador[i][1]!='eliminado' and puede_pedir(cartas,jugador[i][4]):
                        estado=int(input('\tQue quieres hacer:\n\t\t1)Plantarte\n\t\t2)Seguir '))
                        if estado==1:
                            jugador[i][1]='plantado'
                            break
